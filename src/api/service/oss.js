...
module.exports = class extends think.Service {
     ...
     * @param {string} folder - OSS目录
     * @returns {{url: string, key: string}}
     */
    async uploadLocalFile(filePath, contentType = 'application/octet-stream', folder = 'avatar') {
        try {
            const ext = contentType && contentType.indexOf('/') > -1
                ? contentType.split('/')[1]
                : 'jpg';
            ...
            return {
                key,
                url: domain ? `${domain}/${key}` : result.url
            };
        } catch (error) {
            console.error('本地文件上传OSS失败:', error);
            throw error;
        }
    }
